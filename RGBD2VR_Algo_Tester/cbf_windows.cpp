...
void calc_pyr_sizes(int* heights, int* widths, int* pyr_offsets, int inW, int inH, int ScaleNum) {
  int offset = 0;
  for (int scale = 0; scale < ScaleNum; ++scale) {
    pyr_offsets[scale] = offset;
		
    // Calculate the size of the downsampled images.
    heights[scale] = static_cast<int>(inH / pow((float)2, scale));
    widths[scale] = static_cast<int>(inW / pow((float)2, scale));
...
int get_pyr_size(int* heights, int* widths, int scaleNum) {
  int total_pixels = 0;
...
static void savePGM(bool* imf, const char *name, int height, int width) {
  ...
  uint8_t* im = new uint8_t[height * width];
...
void cbf::cbf(uint8_t* depth, uint8_t* intensity,
              ...
              double* sigma_s, double* sigma_r, int width, int height) {
  ...
  int scaleNum = NUM_SCALES;
  calc_pyr_sizes(&pyr_heights[0], &pyr_widths[0], &pyr_offsets[0], width, height, scaleNum);
  ...
  int pyr_size = get_pyr_size(&pyr_heights[0], &pyr_widths[0], scaleNum);
